...
import 'package:track/features/expense/domain/use_cases/category_use_cases/get_categories.dart';
import 'package:track/features/expense/domain/use_cases/category_use_cases/modify_category.dart';
...
class CategoriesBloc extends Bloc<CategoriesEvent, CategoriesState> {
  ...
  Future<void> _onLoad(
      _CategoriesLoad event, Emitter<CategoriesState> emit) async {
    emit(const CategoriesState.loading());

    logger.info('Loading categories for user: ${event.uid}',
        tag: 'CategoriesBloc');
  ...
  Future<void> _onAdd(
      _CategoriesAdd event, Emitter<CategoriesState> emit) async {
    logger.info('Adding category: ${event.category.name}',
        tag: 'CategoriesBloc');
    final stopwatch = Stopwatch()..start();
  ...
  Future<void> _onUpdate(
      _CategoriesUpdate event, Emitter<CategoriesState> emit) async {
    logger.info('Updating category: ${event.category.name}',
        tag: 'CategoriesBloc');
    final stopwatch = Stopwatch()..start();
  ...
  Future<void> _onDelete(
      _CategoriesDelete event, Emitter<CategoriesState> emit) async {
    logger.info('Deleting category: ${event.categoryId}',
        tag: 'CategoriesBloc');
    final stopwatch = Stopwatch()..start();

    final result = await _deleteCategory.call(
        categoryId: event.categoryId, uid: event.uid);
  ...
  Future<void> _onReload(
      _CategoriesReload event, Emitter<CategoriesState> emit) async {
    logger.info('Reloading categories for user: ${event.uid}',
        tag: 'CategoriesBloc');
    final stopwatch = Stopwatch()..start();
  ...
  Future<bool> isCategoryInUse(
      {required int categoryId, required String uid}) async {
    logger.info('Checking if category is in use: $categoryId',
        tag: 'CategoriesBloc');
    final stopwatch = Stopwatch()..start();

    final result =
        await _isCategoryInUse.call(categoryId: categoryId, uid: uid);
